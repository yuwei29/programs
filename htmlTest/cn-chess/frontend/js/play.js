...
play.clickMan = function (key,x,y){
	...
	if (play.nowManKey&&play.nowManKey != key && man.my != com.mans[play.nowManKey ].my){
		...
		if (play.indexOfPs(com.mans[play.nowManKey].ps,[x,y])){
			if(play.depth>3){play.sendToB(com.mans[play.nowManKey].x,com.mans[play.nowManKey].y,x,y)}
			man.isShow = false;
...
play.sendToB = function(x1,y1,x2,y2){
	axios.post('http://localhost:5000/playerOnline',{
		x1,
		y1,
		x2,
		y2
...
play.receiveFromB = async function () {
	const {data} = await axios.get('http://localhost:5000/ai')
	//console.log(data)
	return data
}

play.restartB = function(){
